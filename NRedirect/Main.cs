...
namespace NRedirect
{
	...
	{
		private static string AppDomainName { get; set; } = "NRedirectDomain";
		...
		public static void Start()
		{
			...
			try
			{
				...
				// ANNENDUM: Setting it to an empty string can cause FileIOPermission.CheckIllegalCharacters to shit itself when trying to check for permission when attempting to access the non-existent file.
				// This seems to be the case in select environments:
				// - SMAPI+Stardew Valley : .NET 4.5
				...
				setupInfo.ConfigurationFile = "invalid.exe.config";

				var domain = AppDomain.CreateDomain(AppDomainName, new Evidence(), setupInfo);
				...
				domain.ExecuteAssembly(ConfigurationManager.AppSettings["Executable"], new string[0]);
